...
function DogProvider(props) {
  const [dogState, dogDispatch] = React.useReducer(dogReducer, {dogName: ''})
  const value = [dogState, dogDispatch]
...
function DogNameInput() {
  ...
  const [dogState, dogDispatch] = useDogState()
  const {dogName} = dogState

  function handleChange(event) {
    const newDogName = event.target.value
    // 🐨 change this to call your state setter that you get from useState
    dogDispatch({type: 'TYPED_IN_DOG_INPUT', dogName: newDogName})
  }
